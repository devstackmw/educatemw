    ...
    return (
      ...
      >
        <div className="bg-white/10 p-8 rounded-[3rem] border border-white/10 max-w-sm w-full space-y-6">
          ...
          <div>
            <h2 className="text-4xl font-heading font-black mb-2 tracking-tight">Quiz Finished!</h2>
            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Your Performance</p>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded-3xl border border-white/5">
              <p className="text-3xl font-heading font-black text-indigo-400">{score}/{questions.length}</p>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mt-1">Score</p>
            </div>
            <div className="bg-white/5 p-4 rounded-3xl border border-white/5">
              <p className="text-3xl font-heading font-black text-emerald-400">+{earnedPoints}</p>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mt-1">Points</p>
            </div>
          </div>
          ...
          <AnimatePresence>
            {newBadge && (
              <motion.div 
                ...
                className="bg-gradient-to-r from-purple-600 to-indigo-600 p-4 rounded-3xl flex items-center gap-4 text-left shadow-lg"
              >
                <div className="bg-white/20 p-3 rounded-2xl">
                  <Award size={24} className="text-white" />
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-indigo-200">New Badge Earned!</p>
                  <p className="font-heading font-black text-lg text-white leading-tight">{newBadge}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
  ...
  if (questions.length === 0) return (
    <div className="fixed inset-0 z-50 bg-slate-50 flex items-center justify-center">
      <div className="text-center space-y-4">
        ...
        <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">Preparing Questions...</p>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 bg-slate-50 flex flex-col font-sans">
      ...
      <div className="bg-white border-b border-slate-100 p-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex flex-col items-center">
          ...
          <h2 className="font-bold text-slate-900 text-sm">{quiz.topic}</h2>
        </div>
        <div className="flex items-center gap-2 font-mono bg-slate-900 text-white px-4 py-2 rounded-full text-sm font-bold shadow-sm">
          <Timer size={16} className="text-indigo-400" /> {formatTime(timeLeft)}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 max-w-2xl mx-auto w-full">
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">
          ...
          <h3 className="text-3xl font-heading font-black text-slate-900 leading-tight tracking-tight">
            {questionText}
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {options.map((option, idx) => {
            ...
            let buttonClass = "border-slate-200 bg-white text-slate-700 hover:border-indigo-300 hover:bg-indigo-50/50 shadow-sm";
            ...
            if (showFeedback) {
              ...
              buttonClass = "border-indigo-600 bg-indigo-50 text-indigo-900 shadow-indigo-100";
      ...
      {/* Footer Controls */}
      <div className="p-6 bg-white border-t border-slate-100 flex gap-3 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.05)]">
        {!showFeedback ? (
          ...
          <>
            {currentQuestionIndex < questions.length - 1 ? (
              <button 
                ...
                className="w-full py-4 rounded-2xl font-black bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-600/20 transition-all active:scale-95 flex items-center justify-center gap-2"
              >
              ...
              <button 
                onClick={finishQuiz}
                className="w-full py-4 rounded-2xl font-black bg-emerald-600 hover:bg-emerald-700 text-white shadow-xl shadow-emerald-600/20 transition-all active:scale-95"
              >
                Finish & See Results
              </button>
            )}
