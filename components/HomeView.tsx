...
export default function HomeView({ onNavigate, user, isPremium, onOpenSidebar }: { onNavigate: (tab: string) => void, user?: FirebaseUser | null, isPremium?: boolean, onOpenSidebar: () => void }) {
  ...
  return (
    <div className="p-4 pt-16 space-y-6 pb-28 max-w-2xl mx-auto">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-3">
            ...
            <div className="space-y-0.5">
              <h2 className="text-xl font-bold tracking-tight leading-tight flex items-center gap-2 flex-wrap">
                Muli bwanji, <br/>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => window.dispatchEvent(new CustomEvent('navigate', { detail: 'ai' }))} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold text-[10px] shadow-lg shadow-blue-600/20 active:scale-95 transition-all flex items-center gap-1.5 group">
                Ask Cleo AI
      ...
      {/* Quick Stats */}
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white p-3 rounded-lg border border-slate-100 shadow-sm flex items-center gap-3 cursor-pointer active:scale-95 transition-all" onClick={() => window.dispatchEvent(new CustomEvent('navigate', { detail: 'leaderboard' }))}>
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center px-1">
          ...
          <button onClick={() => window.dispatchEvent(new CustomEvent('navigate', { detail: 'papers' }))} className="text-blue-600 text-[9px] font-bold uppercase tracking-widest">View All</button>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <BentoCard 
